...
class MikrotikStatic
{
    ...
    // show Description of nuxbill plugin in admin
    function description()
    {
        return [
            'title' => 'Mikrotik STATIC',
            'description' => 'To handle connection between PHPNuxBill with Mikrotik STATIC IP',
            'author' => 'Gerandonk',
            'url' => [
    ...
    function add_customer($customer, $plan)
    {
        ...
        if (empty($cid)) {
			...
			$setRequest->setArgument('target', $customer['static_ip']);
			...
			if (!$isExp){
				$this->removeIpFromAddressList($client, $customer['static_ip']);
			}
        }
    }
    ...
    function remove_customer($customer, $plan)
    {
        ...
        if (!empty($plan['plan_expired'])) {
            ...
            if($p){
                ...
                $this->addIpToAddressList($client, $customer['static_ip'], $listName = 'Expired', $customer['username']);
                return;
            }
        }
		$this->addIpToAddressList($client, $customer['static_ip'], $listName = 'Expired', $customer['username']);
